...
Public Class LogFileENG
    ...
    Const AppLogFolder As String = "C:\Application\TESCO-TMS3\ServiceLog\ErrorLog\"
...
#Region "User Activity Log"
    ...
    Public Shared Sub LogTrans(LogMsg As String)

        Dim frame As StackFrame = New StackFrame(1, True)
        Dim ClassName As String = frame.GetMethod.ReflectedType.Name
        Dim FunctionName As String = frame.GetMethod.Name
        Dim LineNo As Integer = frame.GetFileLineNumber
        CreateWindowServiceLog(ClassName, FunctionName, LineNo, LogMsg, AgentLogType.TransLog)
    End Sub



    Public Shared Sub LogError(LogMsg As String)
        Dim frame As StackFrame = New StackFrame(1, True)
        Dim ClassName As String = frame.GetMethod.ReflectedType.Name
        Dim FunctionName As String = frame.GetMethod.Name
        Dim LineNo As Integer = frame.GetFileLineNumber

        CreateWindowServiceLog(ClassName, FunctionName, LineNo, LogMsg, AgentLogType.ErrorLog)
    End Sub


    Public Shared Sub LogException(ExMessage As String, ExStackTrace As String)
        Dim frame As StackFrame = New StackFrame(1, True)
        Dim ClassName As String = frame.GetMethod.ReflectedType.Name
        Dim FunctionName As String = frame.GetMethod.Name
        Dim LineNo As Integer = frame.GetFileLineNumber

        CreateWindowServiceLog(ClassName, FunctionName, LineNo, "Exception : " & ExMessage & vbNewLine & ExStackTrace, AgentLogType.ExceptionLog)
    End Sub
    ...
    Private Shared Sub CreateWindowServiceLog(ClassName As String, FunctionName As String, LineNo As Int16, LogMsg As String, LogType As Int16)
        ''### Current Class and Function name
        'Dim m As MethodBase = MethodBase.GetCurrentMethod()
        'Dim ThisClassName As String = m.ReflectedType.Name
        'Dim ThisFunctionName As String = m.Name
        ...
        Try
            ...
            Dim ret As ExecuteDataInfo = lnq.InsertData(ClassName, trans.Trans)
